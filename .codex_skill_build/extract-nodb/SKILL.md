---
name: extract-nodb
description: Extract business rules from PL/SQL files in an `IN/` directory using the ABRT v1.12 specification, write fresh `_abrt.json` files to `OUT/`, perform validation and gap analysis, and move processed inputs to `DONE/` without loading a database. Use when the user wants ABRT JSON output only from Oracle PL/SQL source files (`.sql`, `.pkb`, `.trg`).
---

# ABRT Extraction Pipeline (No DB)

Use this skill when the user wants to process one or more Oracle PL/SQL `.sql`, `.pkb`, or `.trg` files from `IN/` into ABRT JSON without any MySQL or other database load step. This is the Codex version of the Claude skill named `extract_nodb`.

## Required Inputs

- A working directory containing `IN/`, `OUT/`, and `DONE/`
- Source `.sql`, `.pkb`, or `.trg` files in `IN/`
...
If the directories do not exist, create them before processing. If `IN/` has no `.sql`, `.pkb`, or `.trg` files, report that and stop.
...
1. Scan `IN/` for `.sql`, `.pkb`, and `.trg` files and sort them deterministically.
...
3. For each file:
   ...
   - Perform structural validation of the JSON output against the ABRT v1.12 spec after the gap analysis.
...
- Each child worker must perform the extraction itself for its assigned `IN/*.{sql,pkb,trg}` file during the current run.
- Each generated `OUT/*.json` file must be freshly generated from the current SQL source, followed by a gap analysis against `ABRT_PL-SQL_Business_Rules.md`, and then structurally validated against the ABRT v1.12 grammar before the file is treated as complete.
...
- Populate a useful plain-English `description` on every `BUSINESS_OPERATION` so a human can understand the business purpose of the procedure, function, or trigger without reading the source.
- Populate a useful plain-English `description` on every `BUSINESS_RULE` so a human can understand what the rule does and why it matters.
...
- Model `CONDITION` nodes using the v1.12 explicit forms:
...
After writing each JSON file, perform a gap analysis against `ABRT_PL-SQL_Business_Rules.md` and then perform structural validation of the generated JSON against the ABRT v1.12 spec.
...
6. `abrt_version` is `1.12`.
7. Every `BUSINESS_OPERATION` has a non-empty human-readable `description`.
8. Every `BUSINESS_RULE` has a non-empty human-readable `description`.
9. Every `CONDITION` node conforms to one of the v1.12 explicit forms:
...
10. No `CONDITION` node mixes leaf and compound forms in a way the v1.12 grammar forbids.
11. Every `ACTION.action_type` value matches the v1.12 enum, including `EXIT_LOOP` where loop exits are represented explicitly.
