...
3. For each file:
   ...
   - Perform structural validation of the JSON output against the ABRT v1.11 spec after the gap analysis.
...
- Each generated `OUT/*.json` file must be freshly generated from the current SQL source, followed by a gap analysis against `ABRT_PL-SQL_Business_Rules.md`, and then structurally validated against the ABRT v1.11 grammar before the file is treated as complete.
...
- Model `CONDITION` nodes using the v1.11 explicit forms:
...
After writing each JSON file, perform a gap analysis against `ABRT_PL-SQL_Business_Rules.md` and then perform structural validation of the generated JSON against the ABRT v1.11 spec.
...
6. `abrt_version` is `1.11`.
7. Every `CONDITION` node conforms to one of the v1.11 explicit forms:
...
8. No `CONDITION` node mixes leaf and compound forms in a way the v1.11 grammar forbids.
