...
- **ABRT specs**: `ABRT_PL-SQL_Business_Rules.md` (v1.1), `ABRT_Oracle_Forms3_Extension.md` (v2.0)
...
3. Spawn a **background** child agent per file, up to the max concurrent limit. When at the limit, wait for a child to complete before spawning the next.
4. When a child agent responds:
   - Move the `.sql` file from `IN/` to `DONE/`
   - If at the concurrency limit and files remain, spawn the next child
...
>
> ### Step 5: Update process log
> UPDATE abrt_process_log SET finished = NOW(), business_rules_count = {count}, infrastructure_count = {infra_count} WHERE seq_no = {seq_no}
>
> ### Step 6: Return completion
> Return: file_name, business_rules_count, infrastructure_count, DB load confirmation
