...
3. For each file:
   ...
   - Validate the produced JSON against the ABRT v1.7 grammar and required fields.
...
- Each generated `OUT/*.json` file must be freshly generated from the current SQL source and followed by a gap analysis against `ABRT_PL-SQL_Business_Rules.md` before the file is treated as complete.
...
After writing each JSON file, perform structural validation and then a gap analysis against `ABRT_PL-SQL_Business_Rules.md`.
...
6. `abrt_version` is `1.7`.

If any difference is found, treat that file as a failure and do not mark it complete.

The gap analysis must evaluate the newly generated JSON for that file, not a previously saved artifact.
...
- Move the processed source to `DONE/` only after validation and gap analysis pass.
